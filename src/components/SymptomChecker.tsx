...
export default function SafeFoodPage() {
  ...
  const getStatusColor = (safe: string) => {
    if (safe === 'yes') return 'bg-green-100 border-green-400';
    if (safe === 'no') return 'bg-red-100 border-red-400';
    return 'bg-yellow-100 border-yellow-400';
  };

  const getStatusBadge = (safe: string) => {
    if (safe === 'yes') return 'bg-green-500 text-white';
    if (safe === 'no') return 'bg-red-500 text-white';
    return 'bg-yellow-500 text-white';
  };

  const getStatusText = (safe: string) => {
    if (safe === 'yes') return 'text-green-800';
    if (safe === 'no') return 'text-red-800';
          ...
          <div className="space-y-4 mb-6">
            {results.length === 0 ? (
              ...
              results.map((food, idx) => (
                <div key={idx} className={`p-5 rounded-xl border-2 ${getStatusColor(food.safe)}`}>
                  <div className="flex justify-between items-center mb-2">
                    <h3 className={`font-bold text-xl ${getStatusText(food.safe)}`}>{food.name}</h3>
                    <span className={`text-sm font-bold px-3 py-1 rounded-full ${getStatusBadge(food.safe)}`}>
