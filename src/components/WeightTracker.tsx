...
/* ─────────────────────────────────────────────
   Breed healthy-weight ranges (kg) — dogs & cats
   Format: [min, max]  (ideal adult weight)
───────────────────────────────────────────── */
...
/* ─────────────────────────────────────────────
   BCS (Body Condition Score) mapping 1–9
───────────────────────────────────────────── */
...
function getAgeMonths(dob?: string): number | null {
  if (!dob) return null;
...
/* ─────────────────────────────────────────────
   Rule-based Health Insight Engine
...
function generateHealthInsight(
  ...
  const weeks = Math.max(1, Math.round(
    (new Date(latest.date).getTime() - new Date(oldest.date).getTime()) / (1000 * 60 * 60 * 24 * 7)
  ));
  const weeklyRate = totalChange / weeks;
  const name = petName ?? 'Your pet';
  const parts: string[] = [];

  /* ── 1. Breed range assessment ── */
  if (idealRange) {
    ...
    if (current < minW) {
      const underBy = (minW - current).toFixed(1);
      parts.push(
        `${name} is currently ${underBy} kg below the healthy range for ${breed ? `a ${breed}` : `a ${species}`} (${minW}–${maxW} kg). ` +
        `This level of underweight warrants a vet check to rule out dietary or health issues.`
      );
    } else if (current > maxW) {
      const overBy = (current - maxW).toFixed(1);
      const pctOver = Math.abs(pctFromIdeal).toFixed(0);
      parts.push(
        `${name} is ${overBy} kg (${pctOver}%) above the healthy range for ${breed ? `a ${breed}` : `a ${species}`} (${minW}–${maxW} kg). ` +
        `Consider reducing daily food portions by 10–15% and increasing walk or play time.`
      );
    } else {
      parts.push(
        `Great news — ${name} is within the healthy weight range for ${breed ? `a ${breed}` : `a ${species}`} (${minW}–${maxW} kg). Keep up the excellent care! 🌟`
      );
    }
  }
  ...
  if (weights.length >= 3) {
    ...
    if (Math.abs(weeklyRate) < 0.02) {
      parts.push(`Weight has been very stable over ${weeks} week${weeks !== 1 ? 's' : ''} — a great sign of consistent routine.`);
    } else if (totalChange > 0 && weeklyRate > 0.1) {
      parts.push(
        `${name} has ${direction} ${absDiff.toFixed(2)} kg over ${weeks} week${weeks !== 1 ? 's' : ''} ` +
        `(+${weeklyRate.toFixed(2)} kg/week) — this rate of gain is worth monitoring. ` +
        `A gradual increase is normal for growing pets, but rapid gain in adults may need attention.`
      );
    } else if (totalChange < 0 && weeklyRate < -0.1) {
      parts.push(
        `${name} has ${direction} ${absDiff.toFixed(2)} kg over ${weeks} week${weeks !== 1 ? 's' : ''} ` +
        `(${weeklyRate.toFixed(2)} kg/week). ` +
        (idealRange && current < idealRange[0]
          ? `Combined with being below the ideal range, a vet visit is recommended.`
          : `If this is intentional weight loss, keep it up — slow and steady is healthiest.`)
      );
    } else {
      const sign = totalChange > 0 ? '+' : '';
      parts.push(
        `Over the past ${weeks} week${weeks !== 1 ? 's' : ''}, ${name} has ${direction} ${absDiff.toFixed(2)} kg ` +
        `(${sign}${weeklyRate.toFixed(2)} kg/week) — a healthy, gradual change.`
      );
    }
  } else if (weights.length === 2) {
    const sign = totalChange > 0 ? '+' : '';
    parts.push(
      `Since the first entry, ${name} has ${totalChange > 0 ? 'gained' : 'lost'} ${Math.abs(totalChange).toFixed(2)} kg. ` +
      `Log a few more weights to start seeing meaningful trend analysis.`
    );
  }
  ...
  if (goalWeight !== null) {
    ...
    if (Math.abs(toGo) <= 0.2) {
      parts.push(`🎉 ${name} has reached the goal weight of ${goalWeight} kg — fantastic achievement!`);
    } else if (toGo > 0) {
  ...
  /* ── 4. Fallback if only 1 entry & no breed range ── */
  if (parts.length === 0) {
    parts.push(
      `${name}'s current weight is ${current} kg. ` +
      `Set a breed above to see how this compares to a healthy range, and log more entries to track the trend.`
    );
  }
...
function BCSGauge({ score }: { score: number }) {
  ...
  const colors = ['#f87171','#fb923c','#fbbf24','#a3e635','#34d399','#a3e635','#fbbf24','#fb923c','#f87171'];
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        <div style={{
          ...
          background: 'linear-gradient(90deg, #f87171, #fbbf24, #34d399, #fbbf24, #f87171)',
          position: 'relative',
        ...
        <div style={{ fontSize: '0.75rem', color: info.color, fontWeight: 700, whiteSpace: 'nowrap' }}>
          {info.emoji} BCS {score}/9
        </div>
      </div>
...
function getMilestones(entries: WeightEntry[], goal: number | null): string[] {
  ...
  const lastMonth = sorted.filter(e => {
    const d = new Date(e.date);
    const now = new Date();
    return d >= new Date(now.getFullYear(), now.getMonth() - 1, now.getDate());
  });
  if (lastMonth.length >= 4) badges.push('4+ weigh-ins this month 🔥');
...
export default function WeightTracker({ petId, petInfo, onLatestWeightChange }: Props) {
  ...
  const avgWeight = sorted.length
    ? sorted.reduce((s, e) => s + e.weight, 0) / sorted.length
    : null;

  /* Trend projection (30 days) */
  const chartData = useMemo(() => {
    if (sorted.length < 2) return sorted.map(e => ({ label: fmtDate(e.date), weight: e.weight }));
    const pts = sorted.map((e, i) => ({ x: i, y: e.weight }));
    const reg = linearRegression(pts);
    const base = sorted.map((e, i) => ({
      label: fmtDate(e.date),
      weight: e.weight,
      projected: undefined as number | undefined,
      idealMin: idealRange?.[0],
      idealMax: idealRange?.[1],
    }));
    if (!reg) return base;

    // Append 4 projected points (~30 days)
    const lastDate = new Date(sorted[sorted.length - 1].date);
    for (let i = 1; i <= 4; i++) {
      const d = new Date(lastDate);
      d.setDate(d.getDate() + i * 7);
      const projectedW = parseFloat((reg.slope * (sorted.length - 1 + i) + reg.intercept).toFixed(2));
      base.push({
        label: fmtDate(d.toISOString().slice(0, 10)),
        weight: undefined as any,
        projected: projectedW,
        idealMin: idealRange?.[0],
        idealMax: idealRange?.[1],
      });
    }
  ...
  const bcs = latestWeight && idealRange
    ? estimateBCS(latestWeight, idealRange[0], idealRange[1])
    : null;
    ...
    <>
      <style>{`
        ...
        .wt-root::before {
          ...
          background: linear-gradient(90deg, transparent, rgba(96,165,250,0.6), rgba(147,51,234,0.4), transparent);
        }
        ...
        .wt-title {
          ...
          color: white; letter-spacing: -0.03em; margin-bottom: 2px;
        }
        ...
        .wt-btn-primary:hover { transform: translateY(-1px); box-shadow: 0 6px 20px rgba(59,130,246,0.4); }
        ...
        .wt-btn-ghost:hover { color: rgba(255,255,255,0.7); border-color: rgba(255,255,255,0.15); }
        ...
        @media (max-width: 560px) { .wt-stats { grid-template-columns: repeat(2,1fr); } }
        .wt-stat {
        ...
        .wt-tab:hover { color: rgba(255,255,255,0.55); }
        ...
        .wt-input::placeholder { color: rgba(255,255,255,0.18); }
        .wt-input:focus {
        ...
        .wt-chart-wrap {
          ...
          border-radius: 16px; padding: 1rem 0.5rem 0.5rem;
        ...
        .wt-goal-title {
          ...
          color: rgba(255,255,255,0.22); text-transform: uppercase; letter-spacing: 0.09em;
        }
        .wt-goal-row { display: flex; gap: 0.6rem; align-items: center; }
        ...
        .wt-ai-label {
          ...
          color: rgba(96,165,250,0.7); text-transform: uppercase;
        ...
        .wt-badge {
          ...
          animation: wt-fade-in 0.4s ease;
        }
      ...
      <div className="wt-root">
          ...
          <div className="wt-stats">
            ...
            <div className="wt-stat">
              <div className="wt-stat-num" style={{ color: totalChange === null ? 'white' : totalChange > 0 ? '#f97316' : '#34d399' }}>
                {totalChange !== null ? `${totalChange > 0 ? '+' : ''}${totalChange.toFixed(2)}` : '—'}
        ...
        {/* Goal & breed panel */}
        {showGoal && (
          <div style={{ padding: '1.25rem 2rem', borderBottom: '1px solid rgba(255,255,255,0.06)', background: 'rgba(124,58,237,0.02)', animation: 'wt-fade-in 0.2s ease' }}>
            ...
            <div style={{ display: 'flex', gap: '1.5rem', flexWrap: 'wrap', alignItems: 'flex-end' }}>
              <div className="wt-field">
                ...
                <div style={{ display: 'flex', gap: '0.5rem' }}>
                  ...
                  <button className="wt-btn-primary" type="button" onClick={() => setGoalWeight(parseFloat(goalInput) || null)}>
                    Set
                  </button>
                </div>
              </div>
              <div className="wt-field">
                ...
                <select className="wt-breed-select" value={selectedBreed} onChange={e => setBreed(e.target.value)}>
                  ...
                  <optgroup label="Dogs">
                    {Object.keys(BREED_RANGES).filter(b => !['Domestic Shorthair','Maine Coon','Siamese','Persian','Ragdoll','Bengal','British Shorthair','Abyssinian','Scottish Fold'].includes(b)).map(b => (
        ...
        <div className="wt-tabs">
          <button className={`wt-tab ${activeTab === 'chart' ? 'active' : ''}`} onClick={() => setActiveTab('chart')}>
            Analytics
          </button>
          <button className={`wt-tab ${activeTab === 'log' ? 'active' : ''}`} onClick={() => setActiveTab('log')}>
            History
          </button>
        </div>
                  ...
                  <ResponsiveContainer width="100%" height={230}>
                    <ComposedChart data={chartData} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
                      <defs>
                        ...
                        <linearGradient id="wtIdeal" x1="0" y1="0" x2="0" y2="1">
                      ...
                      <YAxis tick={{ fill: 'rgba(255,255,255,0.2)', fontSize: 11 }} axisLine={false} tickLine={false} domain={['auto','auto']} tickFormatter={v => `${v}`} />
                      ...
                      {goalWeight && (
                        <ReferenceLine y={goalWeight} stroke="rgba(251,191,36,0.5)" strokeDasharray="6 3"
                          label={{ value: `Goal ${goalWeight}kg`, fill: 'rgba(251,191,36,0.7)', fontSize: 10, position: 'right' }} />
                      )}
                      {avgWeight && (
                        <ReferenceLine y={avgWeight} stroke="rgba(255,255,255,0.08)" strokeDasharray="3 3" />
                      )}

                      {/* Area under line */}
                      <Area type="monotone" dataKey="weight" stroke="none" fill="url(#wtBlue)" connectNulls={false} />
                      ...
                      <Line type="monotone" dataKey="projected" stroke="rgba(147,197,253,0.4)" strokeWidth={1.5}
                        strokeDasharray="5 4"
                        dot={{ fill: 'rgba(147,197,253,0.3)', r: 3, strokeWidth: 0 }}
                  ...
                  {idealRange && (
                    <div style={{ marginTop: '0.75rem', fontSize: '0.75rem', color: 'rgba(255,255,255,0.25)' }}>
                      ...
                      {weeksToGoal !== null && goalWeight !== null && (
                        <span style={{ marginLeft: 12, color: 'rgba(251,191,36,0.6)' }}>
                          · ~{weeksToGoal} week{weeksToGoal !== 1 ? 's' : ''} to reach {goalWeight} kg goal
                        </span>
                      )}
                    </div>
                  )}
              ...
              {milestones.length > 0 && (
                <div>
                  ...
                  <div className="wt-milestones">
                    {milestones.map(m => (
                      <div key={m} className="wt-badge">{m}</div>
                    ))}
                  </div>
                </div>
              )}

              {/* Health Insight — rule-based, no API needed */}
              {entries.length >= 1 && (() => {
                const insight = generateHealthInsight(
                  petInfo?.name ?? 'Your pet',
                  petInfo?.species ?? 'pet',
                  selectedBreed || petInfo?.breed,
                  entries,
                  goalWeight,
                  idealRange,
                );
                return (
                  <div className="wt-ai-card">
                    <div className="wt-ai-label">
                      <span>✦</span> Health Insight
                    </div>
                    <div className="wt-ai-text">{insight}</div>
                    {!idealRange && (
                      <div style={{ marginTop: '0.6rem', fontSize: '0.72rem', color: 'rgba(96,165,250,0.4)' }}>
                        💡 Set a breed in Goal settings for breed-specific analysis
                      </div>
                    )}
                  </div>
                );
              ...
              {[...sorted].reverse().map((entry, i, arr) => {
                ...
                return (
                  <div key={entry.id} className="wt-log-item">
                    <div>
                      <div className="wt-log-date">{fmtDate(entry.date)}</div>
                      {entry.notes && <div className="wt-log-notes">{entry.notes}</div>}
                    </div>
                    <div style={{ textAlign: 'right' }}>
                      <div className="wt-log-weight">{entry.weight.toFixed(2)} kg</div>
                      {delta !== null && (
